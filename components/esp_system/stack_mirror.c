...
#if CONFIG_COMPILER_STACK_MIRROR_DEPTH
...
void __cyg_profile_func_enter(void *func, void *caller)
{
    ...
    if (m) {
        if (m->depth < CONFIG_COMPILER_STACK_MIRROR_DEPTH) {
            m->backtrace[m->depth].pc = (uint32_t) __builtin_return_address(0);
            m->backtrace[m->depth].sp = (uint32_t) __builtin_frame_address(0);
        }
        m->depth++;
    }
}
